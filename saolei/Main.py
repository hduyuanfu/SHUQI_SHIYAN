...
class BoomMine:
    ...
    def analyze_block(self, block, left, top):
        x, y = left, top
    ...
    def detect_mine(self, block, left, top):

        def generate_kernel(kernel, kernel_width, kernel_height, left, top):
            ls = []
            loc_x, loc_y = left, top
        ...
        if self.blocks_num[x][y] > 0:
            ...
            to_visit = generate_kernel(kernel, kernel_width, kernel_height, left, top)
    ...
    def process_once(self):
        ...
        self.img, self.blocks_img, self.blocks_x, self.blocks_y, self.block_width, 
        self.block_height, self.left, self.top, self.right, self.bottom = result
        ...
        self.iterate_blocks_image(BoomMine.analyze_block(self.blocks_img[left][top],self.left,self.top))

        # Mark all mines
        self.iterate_blocks_number(BoomMine.detect_mine(self.blocks_num[left][top],self.left,self.top))
